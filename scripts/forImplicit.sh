...
declare -A ImpX20=(
  [cutoffin]="5.0" ## inner cutoff for domain, A
  [cutoffout]="9.0" ## outer cutoff for domain, A
...
measures=(processTraj rate conductivity mechanism)
...
if [[ $measure == "processTraj" ]]; then
  ...
  do
    ...
    do
      ...
      density=${DENSITY[i]}
      box=${BOX[i]}
      ../bin/mechanism $target $density $field $cutoffin $cutoffout $box $box $box $timestep $eqtime  
    done
  done
fi
