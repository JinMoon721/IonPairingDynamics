...
int main(int argc, char* argv[]) {
  ...
  std::string rateFile;
  if(thermoflag==1) rateFile=std::string("../results/rate/") + dirName + "TD" + rho + "E" + fieldname + ".dat";
  else if(thermoflag==2) rateFile=std::string("../results/rate/") + dirName + "TTD" + rho + "E" + fieldname + ".dat";
  else rateFile=std::string("../results/rate/") + dirName + "D" + rho + "E" + fieldname + ".dat";
  ...
  std::cout << std::fixed << std::setprecision(5) << "Density " << density << " M\tField " << field*fconversion << " mV/A\tcutIn "  
    ...
    << "\n";
  */
  auto kr = findKramersRate(innerBoundary, outerBoundary, numatoms, numsnap,10);
  double rateUnit = 1000/timestep; // /ns
  std::cout << "Kramers rate BA Mean : " << kr.mean * rateUnit << " SE : " << kr.error * rateUnit << "\n";
  ...
  auto prod = findqpqm(innerBoundary, outerBoundary);
  std::cout << "committors product\nqpqm\t" << prod.mean<< "\t Error " << prod.error << "\n";


  auto mtptAB = findMTPTAB(innerBoundary, outerBoundary);
  std::cout << "MTPT AB mean : " << 1/mtptAB.mean * rateUnit << "\tError : " << mtptAB.error/mtptAB.mean/mtptAB.mean * rateUnit << "\n";
  std::cout << "compare with " << kr.mean * rateUnit/ prod.mean << "\tError : " << divisionError(kr.mean, kr.error, prod.mean, prod.error) * rateUnit<< "\n";

  // generate output result
  std::string rateFile;
  rateFile=std::string("../results/mtpt/") + dirName + "D" + rho + "E" + fieldname + ".dat";
  std::ofstream out(rateFile );
  out << std::fixed << std::setprecision(8) << density << "\t\t" << field << "\t\t"  
    << CUTOFFin << "\t\t" << CUTOFFout <<"\t\t"  
    << prod.mean << "\t\t" << prod.error << "\t\t"
    << 1/mtptAB.mean * rateUnit << "\t\t" << mtptAB.error/mtptAB.mean/mtptAB.mean * rateUnit << "\t\t" 
    << kr.mean*rateUnit/prod.mean << "\t\t" << divisionError(kr.mean, kr.error, prod.mean, prod.error) * rateUnit
    << "\n";
  out.close();
