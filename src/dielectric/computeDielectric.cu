...
//////////////// END IO
void writeLammpsDumpLiPF6(std::ostream& out, long timestep, const std::vector<Atom>& atoms, const std::vector<int> atomTypes, const Box& box) {
  out << "ITEM: TIMESTEP\n" << timestep << "\n";
...
int main(int argc, char* argv[]) {
  ...
  std::string inputFileName = std::string("../data/dielectric") + dirName + "/videoD" + rho + "E" + fieldname + ".dcd";
  ...
  */
  ...
  for( size_t time = 0; time < numsnap; time++ ) {
    ...
    Dipole systemDipole={0, 0, 0, 0};
    for ( int mol =0; mol < numsolvents; mol++ ) {
      Dipole dipole={0, 0, 0, 0}; 
      for ( int atom =0; atom < oneSolvent; atom++) {
        size_t index = mol*oneSolvent + atom;
        int origin = mol*oneSolvent;
        double dx, dy, dz;
        dx = applyPBC(frame[index].x - frame[origin].x, box.x);
        dy = applyPBC(frame[index].y - frame[origin].y, box.y);
        dz = applyPBC(frame[index].z - frame[origin].z, box.z);
        dipole.x += dx * charges[index];
        dipole.y += dy * charges[index];
        dipole.z += dz * charges[index];
      }
      double musq = dipole.x*dipole.x + dipole.y*dipole.y + dipole.z*dipole.z;
      dipole.mu = std::sqrt(musq);
    //  std::cout << "Molecule " << mol << " Dipole " << dipole.mu << " with " << dipole.x << " " << dipole.y << " " << dipole.z << "\n";

      systemDipole.x += dipole.x;
      systemDipole.y += dipole.y;
      systemDipole.z += dipole.z;
    }
    double totalmusq = systemDipole.x * systemDipole.x + systemDipole.y * systemDipole.y + systemDipole.z * systemDipole.z;
    systemDipole.mu = std::sqrt(totalmusq);
    //std::cout << "snapshot " << time << " total dipole : " << systemDipole.mu << " x " << systemDipole.x << "\n";
    Mx.push_back(systemDipole.x);
    My.push_back(systemDipole.y);
    Mz.push_back(systemDipole.z);

    Mxsq.push_back(systemDipole.x * systemDipole.x);
    Mysq.push_back(systemDipole.y * systemDipole.y);
    Mzsq.push_back(systemDipole.z * systemDipole.z);
    Msq.push_back(systemDipole.mu*systemDipole.mu);
  }
  float Msqmean = mean(Msq);
  float Msqerror = sError(Msq);
  //float Msqsd = std::sqrt( variance(Msq) / static_cast<float>(Msqmean.size())); 
  Dipole total;   // mean dipole vector
  total.x = mean(Mx);
  total.y = mean(My);
  total.z = mean(Mz);

  Dipole totalError;
  totalError.x = sError(Mx);
  totalError.y = sError(My);
  totalError.z = sError(Mz);

  float kBT = 25.6/1000; // eV
  float volume = box.x * box.y * box.z; 
  float epsilon0 = 55.2035/10000; // e^2 /eV /A


  float totalFluctuation = Msqmean - (total.x * total.x + total.y * total.y + total.z * total.z);
  float vecErr = 4* total.x*total.x *totalError.x * totalError.x + 4* total.y*total.y *totalError.y * totalError.y + 4* total.z*total.z *totalError.z * totalError.z ;
  float totalFluctuationE = std::sqrt( Msqerror*Msqerror + vecErr);
  float totalDielectric = 1.0 + totalFluctuation / 3 / epsilon0 / kBT / volume;
  float totalDielectricE = totalFluctuationE/3/epsilon0/kBT/volume;

  float xFluctuation = mean(Mxsq) - total.x*total.x;
  float Ex = std::sqrt( sError(Mxsq)*sError(Mxsq) +  4* total.x * total.x * totalError.x * totalError.x);
  float yFluctuation = mean(Mysq) - total.y*total.y;
  float Ey = std::sqrt( sError(Mysq)*sError(Mysq) +  4* total.y * total.y * totalError.y * totalError.y);
  float zFluctuation = mean(Mzsq) - total.z*total.z;
  float Ez = std::sqrt( sError(Mzsq)*sError(Mzsq) +  4* total.z * total.z * totalError.z * totalError.z);

  Dipole dielectric;
  dielectric.x = 1.0 + xFluctuation /  epsilon0/kBT/volume;
  dielectric.y = 1.0 + yFluctuation /  epsilon0/kBT/volume;
  dielectric.z = 1.0 + zFluctuation /  epsilon0/kBT/volume;
  
  Dipole dielectricE;
  dielectricE.x = Ex/epsilon0/kBT/volume;
  dielectricE.y = Ey/epsilon0/kBT/volume;
  dielectricE.z = Ez/epsilon0/kBT/volume;


  std::cout << "\ntotal dielectric constant : " << totalDielectric << " +- " << totalDielectricE 
    << " x " << dielectric.x << " +- " << dielectricE.x 
    << " y " << dielectric.y << " +- " << dielectricE.y 
    << " z " << dielectric.z << " +- " << dielectricE.z <<   "\n";
