...
void roleAssignment(const std::vector<std::vector<int>>& clusters, std::vector<Atom>& atoms, std::vector<std::vector<int>>& graph) {
  for ( const auto& cl : clusters) {
    ...
    if (static_cast<int>(candidates.size()) == 1) {
      atoms[candidates[0]].role = 1;
    } else {
      std::vector<int> picked = findLeastConnectedIons(take, sign, cl, graph, atoms);
      for (int k : picked) atoms[k].role = 1;
    }
  }
}
...
void updateTag(const std::vector<std::vector<int>>& clusters, std::vector<Atom>& atoms, std::vector<int>& proles, int time) {
  for ( const auto& cl : clusters) {
    int net = netCharge(cl);
    if (net == 0) continue;
...
std::vector<int> detectJump( const std::vector<Atom>& atoms, const std::vector<float>& pdist, float cutoff) {
  std::vector<int> idx;
  for ( int i =0; i<pdist.size(); i++){
    if (std::fabs(atoms[i].nndist - pdist[i]) > cutoff ) {
      idx.push_back(i);
    }
  }
  return idx;
...
void updateNN(
  ...
  std::vector<int> atom2cluster(numatoms, -1);
  ...
  for( int atom=0; atom< numatoms; atom ++) {
    int clusterIdx = atom2cluster[atom];
    const auto& cluster = clusters[clusterIdx];

    int option=-1; // 1: find outside 2: find inside 3: special
      
    if (option == 1) {
      findNearCounterOutside( atom, atoms, clusters, atom2cluster, distanceMatrix);
    } else if (option == 2) {
      findNearCounterInside( atom, atoms, cluster, atom2cluster, distanceMatrix);
    } else {
      ...
    }
    ...
    if (cluster.size() == 1) {
      option = 1;
      if (role[atom] %2 == 0 && role[atom] > 2 ) {
        std::swap(tagMap[atom], tagMap[swap[atom]]);
      }
      role[atom] = 1; // update role as singleton
    }
    // cluster analysis with size 2
    else if (cluster.size() == 2) {
      int charge = netCharge(cluster);
      if (charge == 2) {
        option = 1;
        role[atom] = 1;
      } else if ( charge == -2) {
        ...
        throw std::runtime_error("cluster size2 unknown charge!");
      }
    }

    // cluster with odd atoms
    else if ( cluster.size() % 2 == 1) {
      int charge = netCharge(cluster);

      if (charge == cluster.size() || charge == -cluster.size()) { 
        option = 1;
        role[atom] = 1;
      }
    ...
    // cluster analysis with size 3
      ...
      if (cluster[0]%2==0 && cluster[1]%2==0 && cluster[2]%2==0) { // +++
        ...
        else if ( role[cation] == -1 || role[othercation] == -1 ) { // if this is the first step
          if( cation < othercation ) {
            role[cation]=1;
            role[othercation]=2;
          }
          else {
    ...
    //cluster analysis with size 4
      ...
      if ( cluster[0]%2 + cluster[1]%2 + cluster[2]%2 + cluster[3]%2 == 0 ) { //++++
        ...
        if (anionIdx[cation] == -1 ) std::cout << "fifty\n\n";
      }
      else {
        std::cout << "WHT?\n\n";
      }
    ...
    if (anionIdx[cation] < 0) std::cout << "Final check not work at " << cation << " with cluster size of " << cluster.size() << "\n\n";
  }
  return nndist;
}
*/
...
int main(int argc, char* argv[]) {
  ...
  */
  ...
  int st=40000;
  for (int time= st; time<41000; time++){
    ...
    // Analysis
    if ( time == st ) {
      roleAssignment(clusters, frame, graph); // frame[i].role determines whether it finds counter ion inside or outside of cluster
      ...
      if ( time % 10000 == 0 ) std::cout << "time " << time << "\n";
      roleAssignment(clusters, frame, graph);
      updateTag(clusters, frame,  proles, time);
      //updateTag(pclusters, frame,  proles, time);
      findCounterIons(frame, clusters, distanceMatrix, box);
      for(auto atom : frame) atom.tag = atom.id; // initialize tag
      std::vector<int> jumps = detectJump(frame, pdist, 3.0);
      if (!jumps.empty()) {
        //std::cout << "Jump Occurs at Time " << time << "\n";
        //std::cout << "  Check ";
        for(int mem : jumps){
          if ( frame[mem].nndist < CUTOFFin || pdist[mem] < CUTOFFin ) {
            std::cout <<"Time " << time << "atom " <<  mem << ": " << pdist[mem] << " to "  << frame[mem].nndist << " of " << frame[mem].nncounter << "\n";
          }
        }
      }
