...
std::vector<double> correlation(double rcut, size_t numatoms, size_t numsnap, std::vector<std::vector<double>>& dist, std::vector<int>& interval) {
  std::vector<double> out; // side-side correlation function

  for( int intv : interval) {
    double denom=0; // measure B to A (dissociation), denom = if B(0) / numer = if B(0)A(dt)
    double numer=0;
    for( size_t time=0 ; time < numsnap-intv; time++) {
      for( int atom=0; atom < numatoms; atom++) {
        const bool pdomainA = dist[time][atom] > rcut;
        const bool fdomainA = dist[time+intv][atom] > rcut;
        if (!pdomainA) { 
          denom +=1.0;
          if (fdomainA) {
            numer+=1.0;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Simple-minded Rate Measurement Starts...\n";
  ...
  double rcut = CUTOFFout;
  std::vector<int> interval;
  for(int intv=1; intv < 200000; intv+=500 ) {
    interval.push_back(intv);
  }
  auto sscf = correlation(rcut, numatoms, numsnap, dist, interval);
  std::cout << "side-side time correlation function " << rcut << " done" << std::endl;

  // generate output result
  std::string rateFile;
  rateFile=std::string("../results/singleboundary/") + dirName + "D" + rho + "E" + fieldname + ".dat";

  std::ofstream out(rateFile );
  for( int i = 0 ; i < sscf.size(); i++) {
    out << std::fixed << std::setprecision(8)  << interval[i] * timestep << "\t" << sscf[i] << "\n";
  }
  out.close();
  return 0;
}
